...
export function StepDetails({ outcome }) {
  return (
    <div>
...
export function StepChanges({ wizardData, setWizardData }) {
  ...
  return (
    <div>
      ...
      <DeltaDocInput label="Product Delta Doc" value={wizardData.productDeltaDoc} onChange={updateField('productDeltaDoc')} />
      <DeltaDocInput label="Architecture Delta Doc" value={wizardData.architectureDeltaDoc} onChange={updateField('architectureDeltaDoc')} />
      <DeltaDocInput label="Service Vision Delta Doc" value={wizardData.serviceVisionDeltaDoc} onChange={updateField('serviceVisionDeltaDoc')} />
    </div>
  );
}
...
function DeltaDocInput({ label, value, onChange }) {
  return (
    <Form.Group className="mb-3">
      ...
      <Form.Control type="url" placeholder="https://confluence.example.com/..." value={value} onChange={onChange} />
    </Form.Group>
  );
}

export function StepQuestionnaire({ wizardData, setWizardData }) {
  ...
  return (
    <div>
      ...
      <YesNoSelect label="Does this change impact data architecture?" value={wizardData.questionnaire.impactsData} onChange={updateQuestion('impactsData')} />
      <YesNoSelect label="Does this change have security implications?" value={wizardData.questionnaire.impactsSecurity} onChange={updateQuestion('impactsSecurity')} />
      <YesNoSelect label="Does this change impact accessibility?" value={wizardData.questionnaire.impactsAccessibility} onChange={updateQuestion('impactsAccessibility')} />
      <YesNoSelect label="Does this require architecture review?" value={wizardData.questionnaire.requiresArchReview} onChange={updateQuestion('requiresArchReview')} />
      <Form.Group className="mb-3">
        <Form.Label>Deployment strategy</Form.Label>
        <Form.Control as="select" value={wizardData.questionnaire.deploymentStrategy} onChange={(e) => updateQuestion('deploymentStrategy')(e.target.value)}>
...
export function StepRequest({ wizardData, guildSmes, toggleGuild }) {
  return (
    <div>
