  ...
  return (
    <div className="mb-3 p-3 border rounded bg-light">
      ...
      <div className="d-flex">
        ...
        />
        <Button size="sm" variant="primary" onClick={handleAdd} disabled={!docType || !docUrl}>
          + Add
        </Button>
      </div>
    </div>
...
function AddedDocsTable({ docs, onRemove }) {
        ...
        {paginatedData.map(doc => (
          <tr key={doc.type}>
            ...
            <td className="text-center align-middle">
              <Button variant="link" size="sm" className="p-0 text-danger" onClick={() => onRemove(doc.type)} title="Remove">
                <DeleteIcon />
              </Button>
            </td>
          </tr>
  ...
  return (
    <>
      ...
      <PaginatedTableWrapper data={docs} itemsPerPage={10} itemLabel="docs" renderTable={renderTable} />
    </>
  );
}
...
function DocStatus({ missingTypes }) {
  ...
  return (
    <Alert variant="warning" className="py-2">
      <small><strong>Missing:</strong> {missingTypes.join(', ')}</small>
    </Alert>
  );
}
