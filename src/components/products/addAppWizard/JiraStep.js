...
function JiraLookupSection({ onAdd }) {
  const { searchTerm, setSearchTerm, results, reset, loading, error } = useSearchableAPI('/jira/search');
...
function AvailableJiraTable({ projects, selectedJira, onToggle, onSelectAll }) {
  ...
  return (
    <>
      ...
      <PaginatedTableWrapper data={projects} itemsPerPage={5} itemLabel="projects" renderTable={renderTable} />
    </>
  );
}
        ...
        {paginatedData.map(project => (
          <tr key={project.projectKey}>
            ...
            <td className="text-center align-middle">
              <Button variant="link" size="sm" className="p-0 text-danger" onClick={() => onRemove(project.projectKey)} title="Remove">
                <DeleteIcon />
              </Button>
            </td>
          </tr>
